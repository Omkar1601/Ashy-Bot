...
@bot.event
async def on_ready():
    await bot.change_presence(status = discord.Status.online, activity = discord.Game('still being built'))
	...
	my_db = sqlite3.connect("Farm.sqlite")
	cursor = mydb.cursor()
	cursor.execute("create table if not exists FARM(Farmer_ID text, Buyer_ID text, Card_Name text, Xp_Amount text, Loc text, Price text, Farm_ID text, Farm_Type text)")
